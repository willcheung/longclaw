...
namespace :projects do
	logger = ActiveSupport::Logger.new('log/auto_retrieve_emails.log')
    ...
    Organization.all.each do |org|
    	org.accounts.each do |acc| 
	    	acc.projects.each do |proj|
	    		logger.info "Org: " + org.name + ", Account: " + acc.name + ", Project " + proj.name
	    		ContextsmithService.load_emails_from_backend(proj, logger)
	    	end
	    end
    end
	end
    ...
    Organization.all.each do |org|
    	org.accounts.each do |acc| 
	    	acc.projects.each do |proj|
	    		logger.info "Org: " + org.name + ", Account: " + acc.name + ", Project " + proj.name
	    		ContextsmithService.load_emails_from_backend(proj, logger, after)
	    	end
	    end
    end
	end
