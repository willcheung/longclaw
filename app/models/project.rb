...
class Project < ActiveRecord::Base
  ...
  def daily_activities_last_x_days(time_zone, start_day=14.days.ago.midnight.utc, end_day=Time.current.end_of_day.utc)
    query = <<-SQL
      ...
      WITH time_series as (
        SELECT '#{self.id}'::uuid as project_id, generate_series(date(to_timestamp(#{start_day.to_i}) AT TIME ZONE '#{time_zone}'), date(to_timestamp(#{end_day.to_i}) AT TIME ZONE '#{time_zone}'), INTERVAL '1 day') as days
       )
      (
      ...
      LEFT JOIN (SELECT sent_date, project_id
                 FROM email_activities_last_14d where project_id = '#{self.id}' and EXTRACT(EPOCH FROM (to_timestamp(sent_date::integer) AT TIME ZONE '#{time_zone}')) BETWEEN #{start_day.to_i} AND #{end_day.to_i}
                 ) as activities
      ...
      LEFT JOIN (SELECT last_sent_date as sent_date, project_id
                   FROM activities 
                  WHERE category = '#{Activity::CATEGORY[:Meeting]}' and project_id = '#{self.id}'AND EXTRACT(EPOCH FROM last_sent_date AT TIME ZONE '#{time_zone}') BETWEEN #{start_day.to_i} AND #{end_day.to_i}
                ) as meetings
      ...
      FROM time_series
      LEFT JOIN (SELECT last_sent_date as sent_date, project_id
                  FROM activities where category = '#{Activity::CATEGORY[:JIRA]}' and project_id = '#{self.id}' and EXTRACT(EPOCH FROM last_sent_date AT TIME ZONE '#{time_zone}') > #{start_day.to_i}
                ) as jiras
      ...
      FROM time_series
      LEFT JOIN (SELECT last_sent_date as sent_date, project_id
                  FROM activities where category = '#{Activity::CATEGORY[:Salesforce]}' and project_id = '#{self.id}' and EXTRACT(EPOCH FROM last_sent_date AT TIME ZONE '#{time_zone}') > #{start_day.to_i}
                ) as salesforces
      ...
      FROM time_series
      LEFT JOIN (SELECT last_sent_date as sent_date, project_id
                  FROM activities where category = '#{Activity::CATEGORY[:Zendesk]}' and project_id = '#{self.id}' and EXTRACT(EPOCH FROM last_sent_date AT TIME ZONE '#{time_zone}') > #{start_day.to_i}
                ) as zendesks
      ...
      FROM time_series
      LEFT JOIN (SELECT last_sent_date as sent_date, project_id
                  FROM activities where category = '#{Activity::CATEGORY[:Basecamp2]}' and project_id = '#{self.id}' and EXTRACT(EPOCH FROM last_sent_date AT TIME ZONE '#{time_zone}') > #{start_day.to_i}
                ) as basecamp2s
