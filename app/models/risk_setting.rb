...
class RiskSetting < ActiveRecord::Base
  ...
  def self.create_default_for(organization)
    ...
    create(metric: METRIC[:RAGStatus], medium_threshold: 2, high_threshold: 1, weight: 0.4, is_positive: false, level: organization)
    create(metric: METRIC[:PctNegSentiment], medium_threshold: 0.1, high_threshold: 0.25, weight: 0.3, level: organization)
    create(metric: METRIC[:DaysInactive], medium_threshold: 30, high_threshold: 45, weight: 0.3, level: organization)
