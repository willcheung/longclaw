class UserMailer < ApplicationMailer
  ...
  def daily_summary_email(user)
    ...
    unless sub.blank?
      @updates_today = Project.visible_to(user.organization_id, user.id).following_daily(user.id).preload(:conversations_for_email, :other_activities_for_email, :notifications_for_email)
