class HomeController < ApplicationController
  ...
  def index
    ...
    @open_tasks = Notification.where(project_id: @projects.map(&:id), is_complete: false, conversation_id: visible_activities.map(&:backend_id)).select {|n| visible_activities.any? {|a| n.project_id == a.project_id && n.conversation_id == a.backend_id } }.length
