class HomeController < ApplicationController
  ...
  def index
    ...
    @most_active_projects = Project.visible_to(current_user.organization_id, current_user.id).select("projects.*, count(distinct activities.id) as count_activities").joins(:activities).where("activities.last_sent_date > #{last_7d}").order("count_activities desc").includes([:contacts,:users])
