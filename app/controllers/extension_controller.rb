class ExtensionController < ApplicationController
  ...
  def account
    ...
    @accounts = Account.eager_load(:projects, :user).where("accounts.organization_id = ?", current_user.organization_id).order("upper(accounts.name)") # for account picklist
    ...
    @people_with_profile = people_emails.each_with_object([]) { |e, memo| memo << { email: e, profile: Profile.find_or_create_by_email(e), contact: current_user.organization.contacts.find_by_email(e), name_from_params: (params[:external].blank? || (internal_emails.include? e)) ? nil : (params[:external].values.find{|p| p.second.downcase == e}).first } }
