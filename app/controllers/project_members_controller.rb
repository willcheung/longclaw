class ProjectMembersController < ApplicationController
  ...
  def create
    ...
    emails.each do |email|
      ...
      project_member.status = ProjectMember::STATUS[:Confirmed]
      # If project_member is an (external) SFDC contact, enable _member#show to create an external link
      @salesforce_base_URL = OauthUser.get_salesforce_instance_url(current_user.organization_id) if project_member.contact.present? && project_member.contact.is_source_from_salesforce?
