class ReportsController < ApplicationController
  ...
  def td_user_data
    ...
    tasks_by_complete_date = @user.notifications.where("date(complete_date AT TIME ZONE 'UTC' AT TIME ZONE '#{current_user.time_zone}') >= '#{15.days.ago.midnight}' AT TIME ZONE '#{current_user.time_zone}'").group("date(complete_date AT TIME ZONE 'UTC' AT TIME ZONE '#{current_user.time_zone}')").count
