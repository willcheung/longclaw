class ReportsController < ApplicationController
  def customer
    ...
    ###### Report Data ######
    if !@projects.empty?      
      @project_all_touches = Project.find_include_sum_activities(true, 7*24, @projects.map(&:id))
      ...
      project_prev_all_touches = Project.find_include_sum_activities(true, 7*24, 14*24, @projects.map(&:id))
