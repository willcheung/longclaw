class ReportsController < ApplicationController
  ...
  ACCOUNT_DASHBOARD_METRIC = { :activities_last14d => "Activities (Last 14d)", :days_inactive => "Days Inactive", :negative_sentiment_activities_pct => "Negative Sentiment / Activities %", :open_alerts => "Total Open Alerts", :overdue_tasks => "Total Overdue Tasks", :deal_size => "Deal Size", :days_to_close => "Days to Close"} # Removed: :risk_score => "Risk Score"
  ...
  def ad_sort_data
    ...
    when ACCOUNT_DASHBOARD_METRIC[:activities_last14d]
      ...
      @data = project_engagement.map do |pid, activities|
        ...
        Hashie::Mash.new({ id: proj.id, name: proj.name, deal_size: proj.amount, close_date: proj.close_date, y: activities, total: activities.sum(&:num_activities) })
      end
      #sort_data_by_total = true
      @categories = @data.first.y.map(&:category)
    ...
    when ACCOUNT_DASHBOARD_METRIC[:deal_size]
      ...
      @data = deal_size.map do |t|
        Hashie::Mash.new({ id: t.id, name: t.name, close_date: t.close_date, y: t.deal_size, color: 'default'})
      end
    when ACCOUNT_DASHBOARD_METRIC[:days_to_close]
      days_to_close = projects.select{|p| p.close_date.present? && (p.close_date >= Date.today)}.map{ |p| { id: p.id, name: p.name, deal_size: p.amount, close_date: p.close_date, days_to_close: (p.close_date - Date.today).to_i } }
      @data = days_to_close.map do |t|
        Hashie::Mash.new({ id: t[:id], name: t[:name], deal_size: t[:deal_size], close_date: t[:close_date], y: t[:days_to_close], color: 'default'})
      end
    else # Invalid  
      @data = []
    end
    ...
    # puts "**************** @data: #{@data} \t @categories:  #{@categories} \t ****** @categories_with_data:  #{@categories_with_data}"
    @data = @data.take(25)  # TODO: real left chart pagination
  end
